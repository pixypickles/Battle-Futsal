...
const W=1100,H=700,CX=550,CY=420,RX=350,RY=205,CEILING=520,TEAM={BLUE:0,RED:1};
...
let last=performance.now(),score=[0,0],shake=0,flash=0,banner='BOUNCE RING',resetTimer=0;
const classData={ninja:{name:'NINJA',speed:188,jump:720,air:.92},staff:{name:'STAFF',speed:170,jump:700,air:.76},gauntlet:{name:'FIST',speed:164,jump:710,air:.80}};
const players=[];
...
const human=add(0,'ninja',410,420,true);add(0,'staff',455,340);add(0,'gauntlet',455,500);add(1,'ninja',690,420);add(1,'staff',645,340);add(1,'gauntlet',645,500);
...
const rings=[{team:0,x:CX-RX*.84,y:CY,z:315,axis:'v',label:'BLUE GOAL'},{team:1,x:CX+RX*.84,y:CY,z:315,axis:'v',label:'RED GOAL'}];
function reset(goal=false){const pos=[[410,420],[455,340],[455,500],[690,420],[645,340],[645,500]];players.forEach((p,i)=>Object.assign(p,{x:pos[i][0],y:pos[i][1],z:0,vx:0,vy:0,vz:0,stun:0,hasBall:false,onGround:true,wallJump:true,wallCling:0,wallClingCd:0,jumpCd:0,jumpBuffer:0,airTime:0,kickAnim:0}));Object.assign(ball,{x:CX,y:CY,z:24,vx:(Math.random()<.5?-1:1)*190,vy:rnd(-90,90),vz:85,owner:null,lastTouch:null,trails:[],idle:0,airTime:0,lastX:CX,lastY:CY,lastZ:24,wallHitCd:0});projectiles.length=0;if(goal)flash=1}
function ellipseValue(x,y){return ((x-CX)/RX)**2+((y-CY)/RY)**2}
function confine(o,bounce=.35){let nx=(o.x-CX)/RX,ny=(o.y-CY)/RY,q=nx*nx+ny*ny;if(q<=1)return false;const s=1/Math.sqrt(q);o.x=CX+nx*s*RX;o.y=CY+ny*s*RY;const gx=(o.x-CX)/(RX*RX),gy=(o.y-CY)/(RY*RY),n=norm(gx,gy),dot=o.vx*n.x+o.vy*n.y;if(dot>0){o.vx-=n.x*dot*(1+bounce);o.vy-=n.y*dot*(1+bounce)}return true}
...
function separatePlayers(){for(let i=0;i<players.length;i++)for(let j=i+1;j<players.length;j++){const a=players[i],b=players[j];if(Math.abs(a.z-b.z)>82)continue;let dx=b.x-a.x,dy=b.y-a.y,d=Math.hypot(dx,dy)||.001,min=52;if(d>=min)continue;const push=(min-d)*.52,nx=dx/d,ny=dy/d;a.x-=nx*push;a.y-=ny*push;b.x+=nx*push;b.y+=ny*push;a.vx-=nx*24;b.vx+=nx*24;a.vy-=ny*24;b.vy+=ny*24;confine(a,.1);confine(b,.1)}}
...
function drawArena(){const bg=ctx.createLinearGradient(0,0,0,H);bg.addColorStop(0,'#14283a');bg.addColorStop(.58,'#091624');bg.addColorStop(1,'#03080e');ctx.fillStyle=bg;ctx.fillRect(0,0,W,H);ctx.fillStyle='#0d2330';ctx.beginPath();ctx.ellipse(CX,CY,RX+28,RY+28,0,0,Math.PI*2);ctx.fill();const floor=ctx.createRadialGradient(CX,CY-30,30,CX,CY,470);floor.addColorStop(0,'#315668');floor.addColorStop(.7,'#193746');floor.addColorStop(1,'#0d2431');ctx.fillStyle=floor;ctx.beginPath();ctx.ellipse(CX,CY,RX,RY,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#a4e8ff66';ctx.lineWidth=5;ctx.stroke();ctx.strokeStyle='#d8f7ff2d';ctx.lineWidth=2;for(let r=.25;r<1;r+=.25){ctx.beginPath();ctx.ellipse(CX,CY,RX*r,RY*r,0,0,Math.PI*2);ctx.stroke()}ctx.beginPath();ctx.moveTo(CX-RX,CY);ctx.lineTo(CX+RX,CY);ctx.moveTo(CX,CY-RY);ctx.lineTo(CX,CY+RY);ctx.stroke();
// Ultra-thin height guides make altitude readable without dominating the arena.
ctx.save();ctx.font='9px system-ui';ctx.textAlign='right';ctx.textBaseline='middle';
for(let z=100;z<=500;z+=100){
  const yy=CY-z*.58;
  const width=RX*(.88-z/1800);
  ctx.strokeStyle='rgba(210,247,255,.20)';ctx.lineWidth=.65;ctx.setLineDash([3,7]);
  ctx.beginPath();ctx.moveTo(CX-width,yy);ctx.lineTo(CX+width,yy);ctx.stroke();
  ctx.setLineDash([]);ctx.fillStyle='rgba(225,250,255,.32)';ctx.fillText(String(z),CX-width-7,yy);
}
ctx.restore();
// Tall transparent dome ribs
ctx.save();ctx.strokeStyle='#9de9ff2d';ctx.lineWidth=3;ctx.setLineDash([8,10]);ctx.beginPath();ctx.ellipse(CX,CY-118,RX+10,RY+160,0,Math.PI,Math.PI*2);ctx.stroke();ctx.setLineDash([]);for(const x of [-.7,-.35,0,.35,.7]){ctx.beginPath();ctx.moveTo(CX+x*RX,CY+Math.sqrt(1-x*x)*RY);ctx.quadraticCurveTo(CX+x*RX*.52,CY-400,CX+x*RX,CY-Math.sqrt(1-x*x)*RY);ctx.stroke()}ctx.restore();rings.forEach(drawRing);ripples.forEach(drawRipple)}
